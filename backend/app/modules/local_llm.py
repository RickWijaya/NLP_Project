...
AVAILABLE_LOCAL_MODELS = {
    "gpt2": {
        "name": "GPT-2 Mini",
        "model_id": "gpt2",
        "description": "OpenAI's lightweight GPT-2 124M model, very fast",
        "size_gb": 0.5,
    },
    "tinyllama": {
        "name": "TinyLlama-1.1B",
...
class LocalLLMGenerator:
    ...
    def _format_messages(self, model_key: str, messages: List[Dict[str, str]], tokenizer) -> str:
        ...
        # GPT-2 needs a special simple format - it's a completion model, not chat
        if model_key == "gpt2":
            return self._format_for_gpt2(messages)
        ...
        # Fallback to simple chat format
        prompt = ""
        for msg in messages:
            role = msg["role"]
            content = msg["content"]
    ...
    def _format_for_gpt2(self, messages: List[Dict[str, str]]) -> str:
        """Special format for GPT-2 which is a completion model, not instruction-following."""
        ...
        # Create a simple Q&A completion format

...
class PublicChatRequest(BaseModel):
    """Schema for public chat query (unauthenticated or user-authenticated)."""
    question: str = Field(..., min_length=1, max_length=2000)
    tenant_id: str = Field(..., min_length=1)
    session_id: Optional[str] = None  # Optional session for history
    user_token: Optional[str] = None  # Optional user JWT for authenticated sessions
    user_identifier: Optional[str] = None  # Optional user identifier for tracking
    enable_web_search: bool = False  # Toggle to enable web search
